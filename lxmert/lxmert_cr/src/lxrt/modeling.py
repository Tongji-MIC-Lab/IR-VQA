...
PRETRAINED_MODEL_ARCHIVE_MAP = {
    'bert-base-uncased': "root_path/bert_model/bert-base-uncased.tar.gz",
    'bert-large-uncased': "root_path/bert_model/bert-large-uncased.tar.gz",
    'bert-base-cased': "root_path/bert_model/bert-base-cased.tar.gz",
    'bert-large-cased': "root_path/bert_model/bert-large-cased.tar.gz",
    'bert-base-multilingual-uncased': "root_path/bert_model/bert-base-multilingual-uncased.tar.gz",
    'bert-base-multilingual-cased': "root_path/bert_model/bert-base-multilingual-cased.tar.gz",
    'bert-base-chinese': "root_path/bert_model/bert-base-chinese.tar.gz",
}
...
class LXRTEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.act_scale = VISUAL_CONFIG.act_scale 
        ...
        self.x_layers_loc = nn.ModuleList(
            [LXRTXLayer(config) for _ in range(3)]#self.num_x_layers)]
        )
        self.loc_gate_fc1 = nn.Linear(768, 256)
        self.nl_act = GeLU()
        self.loc_gate_fc2 = nn.Linear(256, 1)
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        #Loc the obj accurately
        lang_feats_loc = lang_feats
        visn_feats_loc = visn_feats
        for layer_module in self.x_layers_loc:
            lang_feats_loc, visn_feats_loc = layer_module(lang_feats_loc, lang_attention_mask,
                                                  visn_feats_loc, visn_attention_mask)
        visn_feats_loc = self.loc_gate_fc1(visn_feats_loc)
        visn_feats_loc = self.nl_act(visn_feats_loc)
        visn_feats_loc_gate = self.loc_gate_fc2(visn_feats_loc)
        visn_feats_loc_gate = visn_feats_loc_gate*self.act_scale
        visn_feats_loc_gate = nn.Sigmoid()(visn_feats_loc_gate)
        visn_feats_pos = visn_feats*visn_feats_loc_gate
        visn_feats_neg = visn_feats*(1-visn_feats_loc_gate)
        lang_feats_pos = lang_feats
        lang_feats_neg = lang_feats#.detach()

        for layer_module in self.x_layers:
            lang_feats_pos, visn_feats_pos = layer_module(lang_feats_pos, lang_attention_mask,
                                                  visn_feats_pos, visn_attention_mask)
            lang_feats_neg, visn_feats_neg = layer_module(lang_feats_neg, lang_attention_mask,
                                                  visn_feats_neg, visn_attention_mask)

        return lang_feats_pos, lang_feats_neg
...
class LXRTModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        # Run LXRT backbone
        lang_feats_pos, lang_feats_neg = self.encoder(
            embedding_output,
            extended_attention_mask,
            visn_feats=visual_feats,
            visn_attention_mask=extended_visual_attention_mask)
        pooled_output_pos = self.pooler(lang_feats_pos)
        pooled_output_neg = self.pooler(lang_feats_neg)

        return pooled_output_pos, pooled_output_neg
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
        ...
        pooled_output_pos, pooled_output_neg = self.bert(input_ids, token_type_ids, attention_mask,
